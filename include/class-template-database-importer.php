...
class Template_Database_Importer {
	...
	/**
	 * API key for templify membership
	 *
	 * @var null
	 */
	private $api_key = '';
	...
	/**
	 * API email for templify membership
	 *
	 * @var string
	 */
	private $api_email = '';
	 ...
	 * @var string
	 */
	protected $templify_import_templates_folder;
	/**
	...
	public function __construct() {
		if ( is_admin() ) {
			// Ajax Calls.
			add_action( 'wp_ajax_templify_import_get_template_data', array( $this, 'template_data_ajax_callback' ) );
			add_action( 'wp_ajax_templify_import_reload_template_data', array( $this, 'template_data_reload_ajax_callback' ) );
		}

		// Add a cleanup routine.
		$this->schedule_cleanup();
		add_action( 'delete_templify_import_templates_folder', array( $this, 'delete_templify_import_templates_folder' ) );
	}
	/**
	...
	public function get_remote_url_contents() {
		...
		$args = apply_filters(
			'templify_importer_get_templates_args',
			array(
	...
	public function get_local_template_data_path() {
		//if ( ! $this->local_template_data_path ) {
			$this->local_template_data_path = get_stylesheet_directory(). '/starter/widget_data.json';
		//}
		return $this->local_template_data_path;
	}

	// public function get_local_template_content_data_path() {
	// 	//if ( ! $this->local_template_data_path ) {
	// 		$this->local_template_data_path = get_stylesheet_directory(). '/starter/content.xml';
	// 	//}
	// 	return $this->local_template_data_path;
	// }
	...
	public function template_data_reload_ajax_callback() {
		...
		// 	wp_send_json( esc_html__( 'No template data', 'templify-starter-templates' ) );
		// } else {
			wp_send_json( $get_data );
		//}
		die;
	...
	public function schedule_cleanup() {
		if ( ! is_multisite() || ( is_multisite() && is_main_site() ) ) {
			if ( ! wp_next_scheduled( 'delete_templify_import_templates_folder' ) && ! wp_installing() ) {
				wp_schedule_event( time(), self::CLEANUP_FREQUENCY, 'delete_templify_import_templates_folder' );
			}
		}
	}
	/**
	 ...
	 * @return bool
	 */
	public function delete_templify_import_templates_folder() {
		return $this->get_filesystem()->delete( $this->get_templify_import_templates_folder(), true );
	}
	/**
	 * Get the folder for templates data.
	 *
