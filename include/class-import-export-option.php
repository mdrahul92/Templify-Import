...
class Customizer_Import_Export {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_templify_import_reset', array( $this, 'ajax_reset' ) );
	}
	/**
	...
	public function enqueue_customizer_scripts() {
		wp_enqueue_style( 'templify-importer-import-export', TEMPLIFY_IMPORT_TEMPLATES_URL . 'assets/css/templify-import-export.css', array( 'wp-components' ), TEMPLIFY_IMPORT_TEMPLATES_VERSION );
		wp_enqueue_script( 'templify-importer-import-export', TEMPLIFY_IMPORT_TEMPLATES_URL . 'assets/export/starter-import-export.min.js', array( 'jquery' ),  TEMPLIFY_IMPORT_TEMPLATES_VERSION, true );
		wp_localize_script(
			'templify-importer-import-export',
	...
	public function ajax_reset() {
		// Check request.
		if ( ! check_ajax_referer( 'templify-import-reseting', 'nonce', false ) ) {
		...
		delete_option( 'templify_global_palette' );
	...
	public static function import_export_setup( $wp_customize ) {
		...
		$wp_customize->add_control( new Templify_Importer_Control_Import_Export( $wp_customize, 'templify_importer_import_export', $control_config ) );
	...
	public static function import_export_requests( $wp_customize ) {
		...
		if ( current_user_can( 'edit_theme_options' ) ) {
			...
			if ( isset( $_REQUEST['templify-importer-import'] ) && isset( $_FILES['templify-importer-import-file'] ) ) {
	...
	private static function import_data( $wp_customize ) {
		...
		if ( ! wp_verify_nonce( $_REQUEST['templify-importer-import'], 'templify-importer-importing' ) ) {
			return;
		}
		...
		// Setup global vars.
		global $wp_customize;
		global $templify_importer_import_error;
		global $wp_filesystem;

		// Setup internal vars.
		$templify_importer_import_error = false;
		$template             = 'templify';
		$overrides            = array( 'test_form' => false, 'test_type' => false, 'mimes' => array( 'dat' => 'text/plain' ) );
		$file                 = wp_handle_upload( $_FILES['templify-importer-import-file'], $overrides );

		// Make sure we have an uploaded file.
		if ( isset( $file['error'] ) ) {
			$templify_importer_import_error = $file['error'];
			return;
		}
	...
	public static function controls_print_scripts() {
		...
		if ( $templify_importer_import_error ) {
			echo '<script>alert("' . esc_js( $templify_importer_import_error ) . '");</script>';
		}
	}
}
